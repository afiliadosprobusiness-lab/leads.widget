...
- Landing principal `/` ahora comunica tres modos de uso: `CRM WhatsApp` (operacion directa en WhatsApp Web), `Widget embebido` y `Lead Chat` como pagina publica sin web.
...
- Landing principal `/` refuerza el posicionamiento de `WhatsWidget` con beneficios operativos concretos: Kanban real, etapas personalizadas, campanas a `opted_in`, notas por contacto, recordatorios con notificaciones locales, respuestas rapidas, importacion CSV al CRM y modo blur para demos.
...
- El bloque `Demos simuladas` de landing principal ahora funciona como sandbox comercial: muestra metricas simuladas `antes/despues`, checklist de onboarding, bloque de cumplimiento/ROI y pipeline interactivo por etapas para `CRM WhatsApp`, `Leads Widget` y `Lead Chat`.
- El sandbox de `Demos simuladas` incluye `mini calculadora ROI` editable por escenario (leads mensuales, tasa de cierre, mejora esperada y ticket promedio) para estimar cierres extra, ingreso adicional y neto mensual vs costo del plan (`S/50` o `S/99`).
...
- En `Demos simuladas`, cada paso del escenario incorpora `mockup guiado` (accion del usuario, actualizacion del sistema y beneficio economico), mas `snapshot monetario` por paso (cierres extra, ingreso desbloqueado, neto, payback y ROI) para clarificar impacto en dinero antes de activar.
...
- En SuperAdmin > `CRM WhatsApp`, la tabla muestra `Prox. renovacion` (fecha de cobro) usando `currentPeriodEnd` remoto y lo persiste en `profiles.whatsapp_crm_current_period_end` para fallback visual.
