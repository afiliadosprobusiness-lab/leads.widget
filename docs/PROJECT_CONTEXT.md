...
- El bloque de `mockups guiados` prioriza una experiencia breve: selector por herramienta, 3 pasos cortos y una maqueta visual no interactiva enfocada en comprension rapida para decisores.
- El mockup de `WhatsWidget` replica visualmente `WhatsApp Web` con referencias a popup y panel de extension; `Leads Widget` muestra la web con popup de salida + widget; `Lead Chat` muestra la pagina publica con handoff a WhatsApp.
