...
- CRM cliente (legacy en leads.widget):
  ...
  - El dashboard cliente mantiene workspace CRM local para `contacts-merge`, `deals`, `tasks` y `timeline`, pero desde 2026-03-20 el listado/alta/detalle/edicion de contactos se resuelve contra backend externo autenticado (`GET|POST /api/crm/contacts`, `GET|PATCH /api/crm/contacts/:contactId`).
  ...
  - Los prospects aprobados en `Adquisicion` se persisten server-side, crean o mergean contacto CRM en backend con `source = acquisition_google_places`, y el frontend refresca/sincroniza el contacto resultante desde `GET /api/crm/contacts/:contactId` o desde el listado CRM del backend.
